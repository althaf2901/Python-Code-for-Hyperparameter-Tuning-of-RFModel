...
import pandas as pd
import numpy as np
df_train= pd.read_csv('C://Users//CCES-14//Desktop//alm//T//axial//cc_rf//T_axial_training.csv')
df_train = df_train[['alpha', 'beta', 'gamma', 'tau','scf_cc']]
df_val = pd.read_csv('C://Users//CCES-14//Desktop//alm//T//axial//cc_rf//T_axial_validation.csv')
df_val = df_val[['alpha', 'beta', 'gamma', 'tau','scf_cc']]
df_test = pd.read_csv('C://Users//CCES-14//Desktop//alm//T//axial//cc_rf//T_axial_testing.csv')
...
#X_test, X_val, y_test, y_val = train_test_split(X_test_all, y_test_all, test_size=0.33, random_state=1000)
y_test
len(y_train)
# Initialize the Sobol sampler
sampler = Sobol()
# Define the search space for the hyperparameters
search_space = [Integer(1, 200, name='n_estimators'),
                Integer(2, 30, name='max_depth'),
                Integer(2, 20, name='min_samples_split'),
                Integer(1, 10, name='min_samples_leaf'),
                Real(0.001, 0.1, name='min_weight_fraction_leaf'),
...
columns = ['n_estimators', 'max_depth', 'min_samples_split', 'min_samples_leaf', 'min_weight_fraction_leaf', 'criterion', 'val_rmse', 'point_type']
...
df.to_csv('C://Users//CCES-14//Desktop//alm//T//axial//cc_rf//axial_cc_rf_dp.csv')
...
req = np.array(df_test['scf_cc']) #for scf_bs
req=req[:].reshape(req.size, 1)
req
len(req)
y_predict_rescale_all
len(y_predict_rescale_all)
q = y_predict_rescale_all
df_test['nn_scf_cc'] = q
df_test.to_csv('axial_cc_rf__database_test.csv')
...
y_predict_rescale_all=np.array(y_predict_rescale_all)
req = np.array(df_val['scf_cc']) #for scf_bs
req=req[:].reshape(req.size, 1)
q = y_predict_rescale_all
df_val['nn_scf_cc'] = q
df_val.to_csv('axial_cc_rf__database_val.csv')
y_predict_train = final_model.predict(X_train)
y_predict_rescale_all = []
min = dfo['scf_cc'].min() #for scf_bs
max = dfo['scf_cc'].max() #for scf_bs
for i in range(0,y_predict_train.size,1):
  y_predict_rescale_all.append((y_predict_train[i]*(max-min))+min)
y_predict_rescale_all=np.array(y_predict_rescale_all)
req = np.array(df_val['scf_cc']) #for scf_bs
req=req[:].reshape(req.size, 1)
q = y_predict_rescale_all
df_train['nn_scf_cc'] = q
df_train.to_csv('axial_cc_rf__database_train.csv')
